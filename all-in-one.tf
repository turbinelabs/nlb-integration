  ...
  units:
    - name: "all-in-one-client.service"
      ...
      content: |
        ...
        ExecStartPre=/usr/bin/docker pull turbinelabs/all-in-one-client:0.13.0
        ExecStart=/usr/bin/docker run --name %n -e 'TBN_COLOR=1B9AE4' -p 8080:8080 turbinelabs/all-in-one-client:0.13.0
  ...
  units:
    - name: "all-in-one-server.service"
      ...
      content: |
        ...
        ExecStartPre=-/usr/bin/docker stop %n
        ExecStartPre=-/usr/bin/docker rm %n
        ExecStartPre=/usr/bin/docker pull turbinelabs/all-in-one-server:0.13.0
        ExecStart=/usr/bin/docker run --name %n -e 'TBN_COLOR=1B9AE4' -p 8080:8080 turbinelabs/all-in-one-server:0.13.0
